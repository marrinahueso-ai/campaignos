...
**Last updated:** July 28, 2026 — Jul 28 closeout: ratings filled + soft-launch status  
...
**Honest Partial areas (usable, not perfect):** Create with AI step count borderline for first-time chairs; event workspace tab density; revision workspace (newsletter/homepage adapters later); Files legacy event embed copy; marketing “Watch demo” link waits for GO; storage capacity gate deferred.

**Next owner actions:** (1) Run launch-checklist finish order A–K on https://heyralli.com. (2) Finish Meta App Review screencast + submit. (3) Invite first pilot org only after Meta green light.
...
| [x] | Quick Actions | Partial | Up Next CTA + Attention deep links; full **Create with AI** / **Calendar** hero CTAs deferred to Ease mockup GO |
...
| [x] | Organization Health | Partial | **Attention** counts (approvals, volunteers, tasks) + optional Insights pulse widget (off by default) |
...
| [ ] | Connected Accounts | Partial | Meta/connect status surfaced via optional Insights widget or Settings — not a dedicated Dashboard tile |
...
| [x] | Customer-facing simplicity | Partial | **Rated Jul 28.** Events Home filters simplified to **Upcoming · Next month · All**. Event detail defaults to Approvals tab — good primary job. **Mall risk:** Calendar month/week/agenda/import vs Events Home vs event tabs (10 tabs) — power density; chairs can live on Events Home + one event tab. |
| [x] | Step count (primary job) | Partial | **Create event:** Events Home → Create → Save ≈ **2–3 steps**. **Import calendar:** Calendar → Import → review → Import All ≈ **4–5 steps** (acceptable for annual job). **Open event workspace:** Events Home → View Details ≈ **2 steps**. |
...
| [x] | School Year Calendar | Wired | `/calendar` month/week/agenda/best-times/import-list; Settings school year. Sub: Meta chips, best-times heatmap, DnD Meta reschedule ([meta-calendar-dnd.md](./meta-calendar-dnd.md)) — not school-event DnD |
| [x] | Calendar import (Google / ICS / file) | Wired | `/calendar?tab=import` (+ aliases); Google OAuth, ICS/webcal, file upload; onboarding Essentials |
| [x] | Calendar review / dedupe | Wired | `/calendar?tab=review`; New/Duplicate/Update/Conflict; [calendar-import-dedupe.md](./calendar-import-dedupe.md); Playwright `14` |
...
Chooser is Social-first + Also available. Flyer mockup-only. AI Edit / AI Copy / AI History are not real customer products.*
...
| [x] | Chooser landing | Wired | `/create-with-ai` — Social-first hero → Creative Setup; quieter **Also available**: Homepage · Volunteer page · Sponsorship (soon) · Newsletter · Flyer (soon) |
...
| [x] | Volunteer page builder | Wired Partial | Soft launch `/volunteer-composer` via Website pages / Also available; share/PDF later |
...
| [x] | Flyer | Coming soon | Chooser card only; mockup at `public/create-with-ai-flyer-ease-mockup.html` — do not ship until GO |
...
| [x] | Activity Timeline | Partial | Event Activity tab exists; classic flows log approval activity; CB2 unified approve/send does not consistently mirror into Activity |
| [x] | Notifications | Wired / N/A | Sidebar Approvals / Changes badges sufficient — no dedicated in-app notification feed needed |
...
| [x] | Drafts | Partial | **Draft** chip on draft-only rows; dedicated **Drafts** filter tab removed from Approvals hub + event tab for soft launch (data unchanged) |
...
| [x] | Checkout | Partial | Wired in code; **launch-checklist 11.4 Needs you** — Owner run once on Production |
...
| [x] | Email Notifications | Wired | Resend delivery for approvals, invites, billing, welcome |
| [x] | Approval Notifications | Wired | Assigned / changes / resubmitted / approved — Account mute prefs |
| [x] | Billing Notifications | Partial | Stripe handles dunning; trial-ending email **Partial** |
...
| Done | Item | Status | Notes |
|------|------|--------|-------|
| [x] | Customer-facing copy | Partial | Transactional emails use org/event language; not every template re-read Jul 28 |
| [x] | Welcome Email | Wired | Magic link CTA **Let's get started** → onboarding |
| [x] | Password Reset | Wired | Supabase recovery flow |
| [x] | Invitation | Wired | Team invite + accept paths |
| [x] | Approval | Wired | Assigned / changes / resubmitted / approved / post kit |
| [x] | Billing Receipt | Wired | Stripe invoice emails + in-app history |
| [x] | Trial Ending | Partial | App trial logic wired; Stripe trial sync — verify copy |
...
| Done | Item | Status | Notes |
|------|------|--------|-------|
| [ ] | Lighthouse 90+ | Partial | Not re-run Jul 28 — [performance-budget.md](./performance-budget.md) |
| [ ] | Mobile Friendly | Partial | Ease shells responsive; not full device matrix |
| [ ] | Accessibility | Partial | Reduced-motion honored in key flows; full audit deferred |
| [x] | Image Optimization | Wired | Next image + artwork pipelines |
| [x] | Lazy Loading | Wired | Standard Next patterns |
| [x] | Caching | Partial | Request cache on dashboard; not fully documented |
| [x] | Database Optimization | Partial | Key query caps (inbox SSR, insights pulse) |
| [x] | CDN Verification | Wired | Vercel edge |
| [ ] | Perf budget (≤2s) | Partial | Budget doc exists; spot-checks only |
...
| [x] | Input Validation | Partial | Server actions + Zod on key paths |
| [x] | SQL Injection Protection | Wired | Supabase / parameterized |
| [x] | XSS Protection | Partial | React defaults; rich HTML export surfaces reviewed |
| [x] | CSRF Protection | Partial | Same-site + server actions |
| [x] | Rate Limiting | Partial | Auth + sensitive actions |
...
| [ ] | Audit remediation open items | Partial | [audit-remediation.md](../security/audit-remediation.md) — non-blockers documented |
