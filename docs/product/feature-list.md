...
- Thread actions: Follow up (star), Done, Delete (with confirm); Assign to team member (active org login users); action buttons use Events KPI card colors (idle `bg-cos-bg-alt` / active `bg-cos-dark`); queue filters Unread / Follow up / Done / Deleted — **shipped**
