...
- Events Home action summary cards (clickable filters: Next 60 Days · Needs Setup · Ready to Run · Needs Follow-up · Done; default Next 60 Days; counts scoped to school-year filter; overlap allowed; per-user drag-and-drop order + color picker via `organization_users.events_home_layout`) — **shipped**
...
- “Files organized by event” carousel cards use Events home summary colors (idle `bg-cos-bg-alt` / selected `bg-cos-dark`) — **shipped**
