...
- Reply composer toolbar: full emoji picker (`emoji-picker-react`, search/categories) + org custom image stickers (upload PNG/WebP/GIF/JPEG to `organization_stickers` / `organization-stickers` bucket; picker shows images; DM send via Meta image attachment) + GIPHY GIF picker (search + trending via server proxy `/api/giphy/*`; rating `pg`; page size 48 with Load more/`offset`; DM-only on `facebook_message` / `instagram_dm`; size-safe CDN URL sent as Meta image attachment like stickers; requires `GIPHY_API_KEY`) + quick emoji pack + 👍/❤️ quick-insert — **shipped**; comment/tag threads stay text-only for stickers/GIFs (clear notice); attachment icon still shows Meta text-only notice for generic files — **shipped**
