...
/**
 * Host Newsletter Composer header artwork so email HTML uses https:// URLs.
 */
export async function uploadNewsletterComposerArtworkAction(input: {
  assetId: string;
  dataUrl: string;
  ...
  const safe = sanitizeEventAssetFilename(input.assetId || "header");
  const ext =
    contentType === "image/png"
      ...
      : contentType === "image/webp"
        ? "webp"
        : "jpg";
  const storagePath = `${organization.id}/newsletter-composer/${safe}-${Date.now()}.${ext}`;
