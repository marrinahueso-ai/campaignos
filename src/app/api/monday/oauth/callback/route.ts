...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      if (exchangeError instanceof MondayClientSecretConfigError) {
        console.error("Monday OAuth client secret config error:", exchangeError.message);
        redirectTarget.searchParams.set("error", "invalid_client_secret_config");
        redirectTarget.searchParams.set("error_description", exchangeError.message);
        return clearOAuthCookies(NextResponse.redirect(redirectTarget), origin);
      }
      throw exchangeError;
    }
    ...
    console.error("Monday OAuth callback failed:", callbackError);
    const redirectTarget = new URL(fallbackReturn, origin);
    redirectTarget.searchParams.set("error", "callback_failed");
    if (callbackError instanceof Error && callbackError.message.trim()) {
      redirectTarget.searchParams.set("error_description", callbackError.message);
    }
    return clearOAuthCookies(NextResponse.redirect(redirectTarget), origin);
  }
}
