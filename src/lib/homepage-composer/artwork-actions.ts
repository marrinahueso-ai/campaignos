...
/**
 * Host Homepage Composer card artwork so Membership Toolkit export uses a
 * normal https:// URL instead of megabytes of base64 in the page HTML.
 */
export async function uploadHomepageComposerArtworkAction(input: {
  cardId: string;
  dataUrl: string;
  ...
  const safeCard = sanitizeEventAssetFilename(input.cardId || "card");
  const ext =
    contentType === "image/png"
      ...
      : contentType === "image/webp"
        ? "webp"
        : "jpg";
  const storagePath = `${organization.id}/homepage-composer/${safeCard}-${Date.now()}.${ext}`;
