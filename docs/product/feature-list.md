...
- **Volunteer Composer** (`/volunteer-composer`) — Volunteer With Us page (Header · Footer · Opportunities · Preview · Export); event picker with month/year + clear; opportunity cards use event artwork when available (emoji fallback); Always on / on/off dates; SignUpGenius URLs; View as parent slider; Open/Coming soon/Closed by as-of; durable local draft; full-page Membership Toolkit HTML export — **Partial** (soft launch; share/PDF later)
