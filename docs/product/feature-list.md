...
- **Flyer (Create with AI)** — **in progress (mockup for daily testing)** — approved ease mockup at [`public/create-with-ai-flyer-ease-mockup.html`](../../public/create-with-ai-flyer-ease-mockup.html): chooser → template picker (10+ locked layouts + custom upload) → inspiration & copy → generate inside template; brand kit toggle, filter chips, slot-fill preview, local draft, print + HTML export; wired from Create with AI chooser (`?view=templates`). Legacy [`public/flyer-composer-mockup.html`](../../public/flyer-composer-mockup.html) redirects here
