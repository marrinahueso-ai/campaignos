...
- Approvals **Request changes** (approver pop-out): terracotta/amber brand accents (distinct from forest open review) — top strip, warm header, **Change request** chip, note + tags, terracotta **Send change request** + Cancel; **← Back to review** reopens open view; full-page `/approvals/revision?mode=approver` remains for deep links — **shipped**
