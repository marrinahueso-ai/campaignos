...
- Approvals Ease hub (UX Pilot): pulse filters + focus card + **Also waiting** queue with **View** only (no approve on rows); How-approvals guide removed from soft-launch hub; `?review=` deep-link reopens open view (approval email invite CTA + Back to review) — **shipped**
