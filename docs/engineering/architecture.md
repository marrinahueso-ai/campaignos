...
**Last updated:** August 19, 2026 — Team & Access overlays portal to document.body  
...
**Settings Ease left nav:** Overview · Organization · Branding · Team & Access · Integrations · Billing & Plan · Account. Header settings gear → `/settings` (no section dropdown). Team & Access pop-outs (`TeamAccessBodyPortal` / `TeamAccessPilotDialog`) mount on `document.body` so `position: fixed` stays viewport-centered — Settings Ease `settings-ease-rise` (transform) otherwise traps overlays in the tall People column. Add member, Role, Create role, and person profile use the same centered dialog (not a full-height right rail). Branding hub (`/settings/branding`) is a 2×2 of equal-height tiles (AI Inbox · Communication Plan · Colors & Logos · School Year); the whole tile is the click (`SETTINGS_EASE_BRANDING_HUB_TILES`). Colors & Logos · School Year nest via `?section=`; AI Inbox and Communication Plan pills/tiles go to `/settings/inbox-ai` and `/settings/playbooks-milestones` (deep `?section=ai-inbox` / `playbook` redirect there). `/settings/inbox-ai` is the Connected Sources list only — no How Inbox AI Works / sources-score / last-refresh chrome. Standalone `/settings/school-year` still works. Colors & Logos previews PTO / school logos via `AppImage` (edit files on Edit branding). Customer **AI Brain** / Training Library UI is unshipped for launch (`/settings/ai-brain` redirects to Branding). Customer **Canva** connect / import UI is unshipped for launch (`/settings/canva` redirects to Integrations; OAuth stays).
...
| Org tone | `getAiProfileByOrganizationId` prompt grounding | DB prefs (empty profile → generic defaults; customer Brain UI unshipped) |
