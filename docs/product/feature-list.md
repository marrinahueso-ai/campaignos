...
**Last updated:** August 6, 2026 — Calendar Select all · Flyer QR/logo/fonts · Social Edit Post scroll
...
- Social **Edit Post** (Preview / change-request): UX Pilot–aligned sheet opened from phone pencil (single edit entry — no competing under-phone Generate link); phone Feed/Story preview left; sage tip + **Artwork | Captions** tabs; What should change + quick chips; **Keep style locked** toggle (defaults on when art exists; persists per post; injects preserve-as-is constraints + high style fidelity on regenerate so only typed instructions change); **Copy notes to Captions** / from Artwork; footer style slider (creative ↔ similar) + **Generate with AI** (first time) / **Regenerate with AI** (when art or caption exists); **Apply & close** in header; generate only sides with instructions; modal body scrolls inside the viewport so Quick suggestions aren’t clipped by the regenerate footer — **shipped**
...
- Header **3 CTA buttons** + footer **2 CTA buttons** (optional labels omitted from preview/export) — **shipped**
