...
**Last updated:** July 29, 2026 — Calendar Ease IA cleanup + import identity hardening
...
- **Flyer (Create with AI)** — **in progress** — Flyer composer at [`public/create-with-ai-flyer.html`](../../public/create-with-ai-flyer.html): **Start** chooser (update last year's flyer upload · three proven layouts — Semester at a Glance, Become an Investor, Event flyer · optional new Letter / half-page simple layout; template lock — AI fills slots, no marketplace grid) → **structured Inspiration slots** (section cards, org, dates/events, location, tiers, CTA, live QR URL + caption, footer, last-year notes) → **Preview Generate** posts full AI direction (`start` path + template + print size + asset metadata + brand kit + every slot field) to `POST /api/flyer-composer/generate` (auth + `upload_artwork` + 1 AI credit); OpenAI returns locked-template slot copy applied to preview; **Semester at a Glance** preview renders month-list rows from `datesEvents` (coalesces calendar lines when AI mis-slots into body); **Directions to AI** summary on Preview sidebar; left-rail stepper (**Start → Inspiration → Preview**); brand kit toggle tints layouts + generated flyers from org palette; slot-fill preview with real QR image; local draft; print + HTML export; wired from Create with AI landing (`?view=start`). Requires http://localhost:3000 or production host (not `file://`). Hero photo / template file bytes not sent in v1 (metadata only); no AI image generation yet. Legacy URLs [`create-with-ai-flyer-ease-mockup.html`](../../public/create-with-ai-flyer-ease-mockup.html) and [`flyer-composer-mockup.html`](../../public/flyer-composer-mockup.html) redirect here
