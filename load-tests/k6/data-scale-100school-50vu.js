/**
 * Data-scale 100-school / 50-VU — staging-only architecture headroom
 * validation against the 100-school / Micro-compute dataset. Same ramp
 * shape as the validated 20-school HEADROOM_50VU profile so concurrency
 * headroom compares directly; uses the 100-school read-only traffic mix
 * and pinned owners across 50 distinct orgs.
 *
 * Purpose: find where 5× data volume begins to show meaningful stress
 * while keeping correctness (auth, tenant isolation, integrity). Not a
 * re-proof of the getClaims auth fix (already confirmed at 20-VU / 20m).
 *
 ...
 *
 * After full preflight, remint the 50 pinned owners (or at least
...
import {
  buildDataScale100School50VuThresholds,
  SUMMARY_TREND_STATS,
} from "./config/thresholds.js";
import {
  DATA_SCALE_100SCHOOL_50VU_WORKLOAD,
...
const PROFILE = "data-scale-100school-50vu";

/** Derived from the workload shape, not hardcoded, so the two can never drift apart. */
const REQUIRED_VUS = Math.max(
  ...DATA_SCALE_100SCHOOL_50VU_WORKLOAD.stages.map((s) => s.target),
);

export const options = {
  scenarios: {
    data_scale_100school_50vu: DATA_SCALE_100SCHOOL_50VU_WORKLOAD,
  },
  thresholds: buildDataScale100School50VuThresholds(),
