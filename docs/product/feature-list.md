...
- Social **Preview** layout: posts rail (Ready / Needs work / Draft + what’s missing) · phone as live Feed/Story preview (caption in chrome) · one **Edit artwork & caption** under phone + quiet Regenerate · right column **How this post goes out** only (platforms · Publish now / Schedule · advanced Story kit email collapsed); header **N of M posts ready** + **Save → Review** (focuses first incomplete + highlights missing setting); app sidebar + composer step rail collapse to icons on Preview/Review — **shipped**
