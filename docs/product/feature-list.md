...
**Last updated:** July 31, 2026 — Flyer Approvals revision UI + Resend copy (print, not feed/story)
...
- **Flyer (Create with AI)** — **in progress** — Product route [`/create-with-ai/flyer`](../../src/app/(dashboard)/create-with-ai/flyer/page.tsx) (dashboard Sidebar + header; embeds [`public/create-with-ai-flyer.html`](../../public/create-with-ai-flyer.html) with `embed=1`; same-origin framing headers allow the iframe; bare HTML redirects into this shell): **Start** path chooser (update last year's flyer · optional proven layouts · new Letter / half-page) with quiet optional **Tied to…** chip (pick event / create-your-own → Events; Social imagery status when bound — concepts from [`flyer-event-files-ease-mockup.html`](../../public/flyer-event-files-ease-mockup.html)) → **Inspiration** centered on Photo · Template file · Brand kit cards, then direction/details → **Preview** calm hierarchy (sage letter stage hero · Edit artwork under image + quiet Edit details · compact Versions · one-line Saved to Files link · slim Tied-to chip + single Approvals CTA in aside · quiet Regenerate/Print/Download in header — Regenerate opens the same Social-style Edit artwork revise-from-selected-version screen as Edit artwork, with instructions/chips/style strength and **Regenerate artwork** CTA that appends a Versions entry; not an instant Inspiration re-roll — no duplicate Update approval / Also / Files buttons); Send for approval → `POST /api/flyer-composer/send-for-approval`; Approvals Edit → `/create-with-ai/flyer?view=result`. Event Files / unbound flows remain UX reference in the mockup (kept live). Wired from Create with AI landing.
