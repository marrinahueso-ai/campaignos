...
**Last updated:** July 30, 2026 — Flyer Preview image persistence + version history
...
- **Flyer (Create with AI)** — **in progress** — Flyer composer at [`public/create-with-ai-flyer.html`](../../public/create-with-ai-flyer.html): **Start** chooser (update last year's flyer upload · optional proven layouts — Semester at a Glance, Become an Investor, Event flyer · new Letter / half-page) → **Inspiration** (artwork direction first, photo as hero/mood, org/dates/location/tiers/CTA/QR/footer) → **Preview Generate** posts direction + facts + brand kit + reference image URLs to `POST /api/flyer-composer/generate` (auth + `upload_artwork` + AI credits); OpenAI image generation returns a polished print flyer (direction-led prompt; photo = scene/hero, prior flyer = optional structure guide); QR composited when URL present; left-rail stepper (**Start → Inspiration → Preview**); brand kit colors; local draft persists fields + inspiration photo + last generated PNG (hosted URL or data URL) across refresh; **Versions** strip under Preview (last ~10 generates as thumbnails; tap restores without re-charging); **Edit artwork** from Preview revises the **selected version** (direction + optional voice + style strength; posts that PNG as the visual reference + change brief to the same generate API — new version lands in the strip); Inspiration remains for first-time details via “Edit details”; print + download; wired from Create with AI landing (`?view=start`). Requires http://localhost:3000 or production host (not `file://`). Legacy URLs [`create-with-ai-flyer-ease-mockup.html`](../../public/create-with-ai-flyer-ease-mockup.html) and [`flyer-composer-mockup.html`](../../public/flyer-composer-mockup.html) redirect here
