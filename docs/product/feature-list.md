...
**Last updated:** August 7, 2026 — Background Library school picker
...
- Owner **Background Library** (`/ops/background-library`) — platform-only curated library: upload inspiration → Generate **10 individual** backgrounds (not a grid) **or bulk upload finished artwork** (up to 40 images / 12MB each; browser uploads via signed Supabase URLs, then registers into review) → approve into collections (Back to School, Fall, Winter, Sports, Generic, Graduation) or reject/delete; archive/restore published; usage count; grids/detail use shared `AppImage` + Supabase Image Transformations (originals stay at `storage_path` / `public_url`); eng: [image-architecture.md](../engineering/image-architecture.md); mockup [`background-library-owner-mockup.html`](../../public/background-library-owner-mockup.html) — **shipped** (owner ops + school picker: Social Creative Setup **Browse Library** and Flyer **Browse Gallery**; search by tag/collection with assortment order for variety; selecting bumps `usage_count`; attaches as normal inspiration — same AI generate cost)
...
- Events Home ease filters (soft pills: **Upcoming** default · **Next month** · **All**; Upcoming shows next-60-days focus/queue; counts scoped to school-year filter + search; status badges on cards only — not filter tabs; focus/queue artwork uses contain thumbs so poster text stays readable after AppImage migration) — **shipped**
...
- **Flyer (Create with AI)** — **partial** — Product route [`/create-with-ai/flyer`](../../src/app/(dashboard)/create-with-ai/flyer/page.tsx) (dashboard Sidebar + header; embeds [`public/create-with-ai-flyer.html`](../../public/create-with-ai-flyer.html) with `embed=1`; same-origin framing headers allow the iframe; bare HTML redirects into this shell): **Start** — **Choose your flyer size** (UX Pilot): three equal cards — **Update last year's flyer** (Fastest + upload) · **Start fresh — Letter** · **Start fresh — Half-page**; horizontal step pills (Start · Inspiration · Preview); optional **Link to an Event** chip (shows **Saved flyers for this event** when bound — Open loads into Preview); quiet **Browse pre-made templates**; forest **Continue** / Cancel — concepts from [`flyer-event-files-ease-mockup.html`](../../public/flyer-event-files-ease-mockup.html) → **Inspiration** — **What’s the vibe?** (UX Pilot): three peer cards (Inspiration Photo · Template File · Brand Kit with Setup logo picker — choose PTO / school / brand-kit logo; selected logo appears in the Brand Kit chip) · vibe textarea · Key Facts (date · time · location · price/URL) · quiet QR “Add link” · Continue to Preview / Save Draft / Back (Browse Gallery deferred) → **Preview** — UX Pilot stage (radial cream well · letter flyer for the selected Versions entry · Social-matching pencil **Edit artwork** on each generated image / Compare pane · **Edit with AI** · **Back** to Inspiration · Print + Download menu · compact step overlay) + right aside (**Tied to** · Versions list — Compare OFF shows the selected version on stage; Compare ON picks two versions side-by-side (first tap Previous, second Current) · **Saved for this event** (list/load prior flyer Files for the bound campaign via `GET /api/flyer-composer/saved?eventId=`) · **Save options** — optional name + **Save to Files** / **Save again to Files** → `POST /api/flyer-composer/save` writes a flyer PNG into the event’s Files tab (`event_playbook_files`, category `flyer`) · **Create next flyer** · To: reviewer · forest **Send for approval**) → **Edit** slide-over drawer (**Edit Flyer**) with **Artwork | Details** tabs (selected version thumb left; refine notes + style chips or event facts right; forest **Update Flyer** regenerates from the selected version and appends a Versions entry — same approval path as Social: edit → regenerate → Send for approval / re-approval); Send for approval → `POST /api/flyer-composer/send-for-approval`; Approvals Edit → `/create-with-ai/flyer?view=result`. Composer chrome uses site typography (**Geist** UI + **Cormorant Garamond** display), matching dashboard chrome. **Event deep link:** `/create-with-ai/flyer?eventId={id}&fresh=1` from the event Create with AI panel (Flyer tile) binds that campaign and starts a clean draft (per-event local drafts). **QR:** first-party `qrcode` — Inspiration preview + letter mockups generate in-browser (`/vendor/qrcode.min.js`); after AI artwork, server always paints a fixed lower-right white square (~11% of shorter side) and stamps a real scannable QR of the same size on every creation (prompt asks for that exact corner/size; no `api.qrserver.com`). Wired from Create with AI landing + event workspace.
...
- Social artwork prompts discourage spreadsheet / calendar-grid / multi-column table layouts unless the user asks for a table or calendar — **shipped**
...
- Footer supports **2 CTA buttons** (optional blank labels are omitted from preview/export) — **shipped**
- Date fields use a **portal popover** (no inline layout expand / blank space) on cards and announcements — **shipped**
