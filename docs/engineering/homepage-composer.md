...
Cards, `selectedEventIds`, announcement bar lines, **and full chrome** (hero/footer copy + colors, up to 3 header CTAs / 2 footer CTAs, `cardsSectionTitle`, resources) are scoped to `workingMonth` (YYYY-MM). The composer shows a persistent **Working on** strip on every step; live state mirrors the active month. **Save this month** writes a full snapshot into `monthSaved` (Copy from… source). Autosave stashes the same shape into `monthDrafts`.
