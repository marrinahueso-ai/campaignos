...
- Owner **Background Library** (`/ops/background-library`) — platform-only curated library: upload inspiration → Generate **10 individual** backgrounds (not a grid) **or bulk upload finished artwork** (up to 40) into the review queue → approve into collections (Back to School, Fall, Winter, Sports, Generic, Graduation) or reject/delete; archive/restore published; usage count; grids/detail use shared `AppImage` + Supabase Image Transformations (originals stay at `storage_path` / `public_url`); mockup [`background-library-owner-mockup.html`](../../public/background-library-owner-mockup.html) — **shipped** (owner ops v1; school picker wiring next; no aspect-ratio lock so assets can inspire flyers + social)
