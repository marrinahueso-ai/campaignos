...
- **Flyer (Create with AI)** — **in progress (mockup for daily testing)** — approved ease mockup at [`public/create-with-ai-flyer-ease-mockup.html`](../../public/create-with-ai-flyer-ease-mockup.html): **Start** chooser (update last year's flyer upload · three proven layouts — Semester at a Glance, Become an Investor, Event flyer · optional new Letter / half-page simple layout; template lock — AI fills slots, no marketplace grid) → **structured Inspiration slots** (section cards, org, dates/events, location, tiers, CTA, live QR URL + caption, footer, last-year notes) → generate inside template; left-rail stepper (**Start → Inspiration → Preview**); brand kit toggle tints layouts + generated flyers from org palette; slot-fill preview with real QR image; local draft; print + HTML export; wired from Create with AI landing (`?view=start`). Legacy [`public/flyer-composer-mockup.html`](../../public/flyer-composer-mockup.html) redirects here
