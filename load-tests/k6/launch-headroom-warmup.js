/**
 * 50-VU headroom warm-up — ~5 VUs for ~2 minutes against the same Vercel
 * Preview deployment used for the recorded headroom runs. Confirms routes,
 * sessions, and Deployment-Protection bypass. Results are discarded.
 *
 *   BASE_URL=… TEST_RUN_ID=… VERCEL_JWT=… npm run test:load:headroom-warmup
 */

import { prepareTestContext, warmAuth } from "./helpers/auth.js";
import { HEADROOM_50VU_WARMUP_WORKLOAD, LIGHT_PEAK_TRAFFIC_WEIGHTS } from "./config/workload.js";
import { runWeightedIteration } from "./scenarios/run-mix.js";

export const options = {
  scenarios: {
    headroom_warmup: HEADROOM_50VU_WARMUP_WORKLOAD,
  },
};
...
export function setup() {
  ...
  console.log(
    `[k6 headroom-warmup] BASE_URL=${data.baseUrl} TEST_RUN_ID=${data.testRunId} ` +
...
export function handleSummary(data) {
  ...
  const authFail = data.metrics.auth_failures
    ...
    : 0;
  console.log(
    `[k6 headroom-warmup] DISCARDED — checks pass rate=${checks ? (checks.rate * 100).toFixed(1) : "?"}% ` +
      `tenant_isolation_failures=${tenantFail} auth_failures=${authFail}. ` +
      `Timing not reported; not one of the 3 recorded headroom runs.`,
  );
  return {};
}
