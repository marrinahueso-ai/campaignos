...
- Owner **Background Library** (`/ops/background-library`) — platform-only curated library: upload inspiration → Generate **10 individual** backgrounds (not a grid) → approve into collections (Back to School, Fall, Winter, Sports, Generic, Graduation) or reject/delete; archive/restore published; usage count; mockup [`background-library-owner-mockup.html`](../../public/background-library-owner-mockup.html) — **shipped** (owner ops v1; school picker wiring next; no aspect-ratio lock so assets can inspire flyers + social)
