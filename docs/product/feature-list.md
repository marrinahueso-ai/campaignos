...
- Nav `/create-with-ai` — Social-first landing (**Start here** Social hero → Creative Setup); quieter **Also available** row: Homepage · Volunteer page · Sponsorship (soon) · Newsletter · Flyer; customer-facing copy uses calm org/team language ([QA](../qa/newsletter-composer.md)); empty/access hub when no events or no permission — **shipped**
...
- **Flyer (Create with AI)** — **in progress** — Product route [`/create-with-ai/flyer`](../../src/app/(dashboard)/create-with-ai/flyer/page.tsx) (dashboard Sidebar + header; embeds [`public/create-with-ai-flyer.html`](../../public/create-with-ai-flyer.html) with `embed=1`; same-origin framing headers allow the iframe; bare HTML redirects into this shell): **Start** path chooser (update last year's flyer · optional proven layouts · new Letter / half-page) with quiet optional **Tied to…** chip (pick event / create-your-own → Events; Social imagery status when bound — concepts from [`flyer-event-files-ease-mockup.html`](../../public/flyer-event-files-ease-mockup.html)) → **Inspiration** centered on Photo · Template file · Brand kit cards, then direction/details → **Preview** calm hierarchy (sage letter stage hero · single **Edit** under image · compact Versions · one-line Saved to Files link · slim Tied-to chip + single Approvals CTA in aside · Print/Download in header — Print opens a dedicated US Letter document with only the generated PNG so Safari does not crop the iframe/composer stage; Download unchanged) → **Edit** pop-out with **Artwork | Details** toggle (selected version left; Artwork notes/chips/style or curated facts right; one **Regenerate artwork** CTA builds a single revise-from-selected-version payload from both panes and appends a Versions entry — not an Inspiration dump or two generate jobs); Send for approval → `POST /api/flyer-composer/send-for-approval`; Approvals Edit → `/create-with-ai/flyer?view=result`. **QR:** first-party `qrcode` — Inspiration preview + letter mockups generate in-browser (`/vendor/qrcode.min.js`); after AI artwork, server detects the blank white QR box and stamps a real scannable code that fills it (prompt asks for a compact slot and forbids fake QR patterns / comic side-rays on type; no `api.qrserver.com`). Event Files / unbound flows remain UX reference in the mockup (kept live). Wired from Create with AI landing.
...
- **Revision workspace (change request)** — **partial** — `/approvals/revision` shell matching [`public/approvals-change-request-mockup.html`](../../public/approvals-change-request-mockup.html) + in-shell AI regenerate per [`public/approvals-revision-ai-regenerate-mockup.html`](../../public/approvals-revision-ai-regenerate-mockup.html). **Social:** creator and approver see **feed (1:1) + story (9:16)**; Approver note prefills **Instruct AI**; regenerate **feed / story / both** or **caption**; tags Artwork · Stories · Date · Caption · Copy. **Flyer** (`channel: flyer` / `flyer-composer:` milestone): single large **print flyer** preview (no story slot) with hover/click enlarge lightbox; flyer lede + tags Artwork · Date · Copy · QR · Layout; Approve (draft-only, not “Approve & schedule”); creator deep-links to Flyer composer (`/create-with-ai/flyer?view=result`) instead of in-shell social regenerate. Approval emails send app-built HTML (`sendEmail`) with artwork thumbnails — not Resend template variables (2,000-char limit + HTML escaping). Flyer vs social copy uses Print flyer · Open Flyer composer labels. Later: deeper multi-round history, adapters (newsletter · homepage · website pages)
...
- Approvals **open review** (replaces cramped drawer): Social side-by-side feed · 1:1 + story · 9:16; Flyer single print preview + **Approve** (draft); schedule hero, caption/on-flyer copy, timeline; **Approve & schedule** / **Request changes** for Social (no note on open view — note + tags on `/approvals/revision?mode=approver`) — **shipped** (mockup: [`public/approvals-open-view-mockup.html`](../../public/approvals-open-view-mockup.html))
...
**Top rail utilities:** Home · Help (`/help`, **?**) · Settings — Ask Ralli is on Help Center only (not sidebar-pinned)
