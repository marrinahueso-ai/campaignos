...
- **Flyer (Create with AI)** — **in progress** — Product route [`/create-with-ai/flyer`](../../src/app/(dashboard)/create-with-ai/flyer/page.tsx) (dashboard Sidebar + header; embeds [`public/create-with-ai-flyer.html`](../../public/create-with-ai-flyer.html) with `embed=1`; same-origin framing headers allow the iframe; bare HTML redirects into this shell): **Start** path chooser (update last year's flyer · optional proven layouts · new Letter / half-page) with quiet optional **Tied to…** chip (pick event / create-your-own → Events; Social imagery status when bound — concepts from [`flyer-event-files-ease-mockup.html`](../../public/flyer-event-files-ease-mockup.html)) → **Inspiration** centered on Photo · Template file · Brand kit cards, then direction/details → **Preview** aligned to that mock (green letter stage · 2:3 art · Edit under image · soft Saved to Files / Tied banner · Campaign + Approvals aside · Versions strip · Generate/Regenerate/Print/Download in header); Send for approval → `POST /api/flyer-composer/send-for-approval`; Approvals Edit → `/create-with-ai/flyer?view=result`. Event Files / unbound flows remain UX reference in the mockup (kept live). Wired from Create with AI landing.
